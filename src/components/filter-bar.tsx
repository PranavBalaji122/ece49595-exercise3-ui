...
export function FilterBar() {
  ...
  return (
    <section
      ...
      className="min-w-0 rounded-2xl border border-line bg-white px-4 py-4 sm:px-5"
    >
      <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
        ...
        >
          {(["All sports", ...sports] as SportFilter[]).map((sport) => (
            <button
              key={sport}
              type="button"
              aria-pressed={filters.sport === sport}
              onClick={() => setFilters({ ...filters, sport })}
              className={`inline-flex min-h-10 items-center gap-2 rounded-lg px-3 py-2 text-xs font-semibold transition ${filters.sport === sport ? "bg-forest text-white" : "bg-[#f6f7f3] text-muted hover:bg-sage hover:text-forest"}`}
            >
              <SportIcon
        ...
        <div className="flex shrink-0 items-center gap-3 border-t border-line pt-3 xl:border-l xl:border-t-0 xl:pl-5 xl:pt-0">
          ...
          <select
            ...
            onChange={(event) =>
              setFilters({ ...filters, distance: Number(event.target.value) })
            }
            className="min-h-10 rounded-lg border border-line bg-white px-2 text-xs font-semibold"
          >
        ...
        <div className="flex items-center gap-4">
          <label className="flex cursor-pointer items-center gap-2 py-1">
            <input
              ...
              onChange={(event) =>
                setFilters({ ...filters, openOnly: event.target.checked })
              }
          ...
          {active && (
            <button
              ...
              className="flex items-center gap-1.5 py-1 font-semibold text-forest"
            >
