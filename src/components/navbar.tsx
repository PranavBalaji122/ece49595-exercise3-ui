...
export function Brand({ light = false }: { light?: boolean }) {
  return (
    <span
      className={`inline-flex items-center gap-2.5 text-[23px] font-extrabold tracking-[-1.2px] ${light ? "text-white" : "text-forest"}`}
    >
      <svg
        width="30"
        height="30"
    ...
    <header className="border-b border-line bg-white">
      <div className="page-shell flex min-h-20 flex-wrap items-center justify-between gap-x-6 gap-y-2 py-4 sm:py-0">
        ...
        <nav
          ...
          className="order-3 flex w-full gap-7 self-stretch sm:order-none sm:w-auto sm:gap-9"
        >
          <Link
            href="/"
            aria-current={pathname === "/" ? "page" : undefined}
            className={`nav-link ${pathname === "/" || pathname.startsWith("/games/") ? "nav-active" : ""}`}
          >
            Discover
          </Link>
          <Link
            href="/my-games"
            aria-current={pathname === "/my-games" ? "page" : undefined}
            className={`nav-link ${pathname === "/my-games" ? "nav-active" : ""}`}
          >
            My games
            {joinedIds.length > 0 && (
              <span className="ml-1.5 rounded-full bg-sage px-1.5 py-0.5 text-xs text-forest">
                {joinedIds.length}
              </span>
            )}
          </Link>
        ...
        <span className="flex items-center gap-2 text-xs font-medium text-muted sm:text-sm">
          ...
          <span className="hidden min-[400px]:inline">West Lafayette, IN</span>
          <span className="min-[400px]:hidden">West Lafayette</span>
        </span>
      </div>
    </header>
  );
